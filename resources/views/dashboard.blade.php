<x-app-layout>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
                ...
                <div class="bg-gray-800 rounded-xl shadow-sm overflow-hidden">
                    ...
                    <div class="pt-14 p-4">
                        <div class="flex items-center justify-between">
                            ...
                            <a href="https://github.com/{{ auth()->user()->githubProfile }}" target="_blank"
                        ...
                        @if (auth()->user()->skills && is_array(auth()->user()->skills))
                            <div class="mt-4 flex flex-wrap gap-2">
                                ...
                                @foreach (auth()->user()->skills as $index => $skill)
                                    <span
                                        class="px-2 py-1 {{ $colors[$index % count($colors)] }} text-white rounded-full text-xs">{{ $skill }}</span>
                                @endforeach
                            </div>
                        @endif
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-gray-800 rounded-xl shadow-sm p-4">
                    ...
                    <div class="flex justify-between mt-4 pt-4 border-t border-gray-700">
                        <a href="{{ route('posts.create', ['content_type' => 'code']) }}"
                            class="flex items-center space-x-2 text-gray-300 hover:text-blue-500">
                        ...
                        <a href="{{ route('posts.create', ['content_type' => 'image']) }}"
                            class="flex items-center space-x-2 text-gray-300 hover:text-blue-500">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <a href="{{ route('posts.create', ['content_type' => 'link']) }}"
                            class="flex items-center space-x-2 text-gray-300 hover:text-blue-500">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <div class="bg-gray-800 rounded-xl shadow-sm">
                            <div class="p-4">
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center">
                                        @if ($post->user->githubProfile)
                                            <img src="https://github.com/{{ $post->user->githubProfile }}.png"
                                                alt="Photo de profil" class="w-10 h-10 rounded-full" loading="lazy" />
                                        @else
                                            <img src="https://avatar.iran.liara.run/public/boy" alt="Photo de profil"
                                        ...
                                        <div class="ml-3">
                                            ...
                                            <p class="text-gray-400 text-sm">{{ $post->created_at->diffForHumans() }}
                                            </p>
                                        </div>
                                    </div>
                                    <div>
                                        @if (auth()->id() === $post->user->id)
                                            <!-- Update and Delete Buttons for the post creator -->
                                            <div class="flex space-x-2">
                                                <!-- Update Button -->
                                                <a href="{{ route('posts.edit', $post->id) }}"
                                                    class="flex items-center text-blue-400 hover:text-white">
                                                    <svg class="w-5 h-5 mr-1" fill="none" stroke="currentColor"
                                                        viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round"
                                                            stroke-width="2"
                                                            d="M11 4H6a2 2 0 00-2 2v5m0 0l7 7a2 2 0 002.828 0l5-5a2 2 0 000-2.828l-7-7m-4 9l4-4" />
                                                    </svg>
                                                    <span>Update</span>
                                                </a>
                                                <!-- Delete Button -->
                                                <form action="{{ route('posts.destroy', $post->id) }}" method="POST"
                                                    ...
                                                    @method('DELETE')
                                                    <button type="submit"
                                                        class="flex items-center text-red-400 hover:text-white">
                                        ...
                                        @else
                                            ...
                                            @if (!$isConnected)
                                                <form action="{{ route('connections.add', $post->user->id) }}"
                                                    method="POST">
                                                    @csrf
                                                    <button type="submit"
                                                        class="flex items-center text-green-400 hover:text-white">
                                                        <svg class="w-5 h-5 mr-1" fill="none"
                                                            stroke="currentColor" viewBox="0 0 24 24">
                                                            <path stroke-linecap="round" stroke-linejoin="round"
                                                                stroke-width="2"
                                                                d="M18 9l3 3-3 3M21 12H9M9 5a7 7 0 0110 0v2M9 5a7 7 0 00-7 7v2a7 7 0 007 7" />
                                                        </svg>
                                                        <span>Connect</span>
                                                    </button>
                                                </form>
                                ...
                                @switch($post->content_type)
                                    @case('image')
                                        <div class="mt-4">
                                            <img src="{{ Storage::url($post->content) }}" alt="" class="rounded-sm"
                                                loading="lazy">
                                        </div>
                                    @break
                                    ...
                                    @case('link')
                                        <a href="{{ $post->project_link }}" target="_blank"
                                            class="mt-2 inline-block bg-blue-500 hover:bg-blue-600 text-white font-semibold py-2 px-4 rounded-lg">
                                            Visit Link
                                        </a>
                                    @break
                                    ...
                                    @case('code')
                                        <div
                                            class="mt-4 bg-gray-900 rounded-lg p-4 font-mono text-sm text-gray-200 overflow-x-auto">
                                ...
                                <div class="flex justify-between mt-4 border-t border-gray-700 pt-4">
                                    <button class="flex items-center text-gray-400 hover:text-blue-500"
                                        ...
                                        <span class="ml-2">{{ $post->comments->count() }}
                                        ...
                                        <div class="flex items-start">
                                            @if (auth()->user()->githubProfile)
                                                <img src="https://github.com/{{ auth()->user()->githubProfile }}.png"
                                                    alt="Avatar" class="w-8 h-8 rounded-full" loading="lazy" />
                                            @else
                                            ...
                                            <div class="ml-2 flex-grow">
                                                <textarea name="content" placeholder="Ajouter un commentaire..."
                                                    class="w-full bg-gray-700 text-gray-200 p-2 border border-gray-600 rounded-lg focus:outline-none focus:border-blue-500 resize-none"></textarea>
                                                <button type="submit"
                                                    class="mt-2 px-4 py-1 text-white bg-blue-500 hover:bg-blue-600 rounded-md text-sm transition duration-200">Publier</button>
                                            </div>
                                        </div>
                                    </form>
                                        ...
                                        @foreach ($post->comments as $comment)
                                            <div class="flex items-start">
                                                @if ($comment->user->githubProfile)
                                                    <img src="https://github.com/{{ $comment->user->githubProfile }}.png"
                                                        alt="Avatar" class="w-8 h-8 rounded-full" loading="lazy" />
                                                @else
                                                    <img src="https://avatar.iran.liara.run/public/boy" alt="Avatar"
                                                        class="w-8 h-8 rounded-full" loading="lazy" />
                                                ...
                                                <div class="ml-2 bg-gray-700 p-3 rounded-lg flex-grow">
                                                    <div class="flex justify-between">
                                                        <span
                                                            class="font-medium text-gray-200">{{ $comment->user->username }}</span>
                                                        <span
            ...
            <aside class="w-80 p-4">
                ...
                <div class="bg-gray-800 rounded-xl shadow-sm p-4 mb-6">
                    ...
                    <ul class="space-y-4">
                        @foreach (\App\Models\Connection::where(function ($query) {
        $query->where('user_id', auth()->id())->orWhere('connection_id', auth()->id());
    })->where('status', 'accepted')->get() as $connection)
                            <li class="flex items-center">
                                @php
                                    // Determine the connected user (the one who is NOT the authenticated user)
                                    $connectedUser =
                                        $connection->user_id == auth()->id()
                                            ? $connection->connectionUser
                                            : $connection->user;
                                @endphp
                                @if ($connectedUser->githubProfile)
                                    <img src="https://github.com/{{ $connectedUser->githubProfile }}.png"
                                        alt="Avatar" class="w-8 h-8 rounded-full" loading="lazy" />
                                @else
                                    <img src="https://avatar.iran.liara.run/public/boy" alt="Avatar"
                                        class="w-8 h-8 rounded-full" loading="lazy" />
                                @endif
                                <div class="ml-3">
                                    <p class="text-white font-medium">{{ $connectedUser->username }}</p>
                                </div>
                                <div class="ml-auto">
                ...
                @php
                    $pendingRequests = \App\Models\Connection::where('connection_id', auth()->id())
                ...
                @if ($pendingRequests->isNotEmpty())
                    <div class="bg-gray-800 rounded-xl shadow-sm p-4">
                        <h3 class="text-xl font-bold text-white mb-4">Connection Requests</h3>
                        <ul class="space-y-4">
                            @foreach ($pendingRequests as $request)
                                <li class="flex items-center justify-between">
                                    <div class="flex items-center">
                                        @if ($request->user->githubProfile)
                                            <img src="https://github.com/{{ $request->user->githubProfile }}.png"
                                                alt="Avatar" class="w-8 h-8 rounded-full" loading="lazy" />
                                        @else
                                            <img src="https://avatar.iran.liara.run/public/boy" alt="Avatar"
                                                class="w-8 h-8 rounded-full" loading="lazy" />
                                    ...
                                    <div>
                                        <form action="{{ route('connections.accept', $request->id) }}" method="POST"
                                            class="inline">
                                            @csrf
                                            @method('PATCH')
                                            <button type="submit" class="text-green-400 hover:text-white text-sm">✔
                                                Accept</button>
                                        </form>

                                        <form action="{{ route('connections.decline', $request->id) }}"
    ...
    `;
                    ...
                    resultsContainer.innerHTML =
                        '<p class="text-gray-400 px-4 py-2">No posts found matching your search.</p>';
                }
            } catch (error) {
                console.error("Search error:", error);
                resultsContainer.innerHTML =
                    '<p class="text-red-400 px-4 py-2">Failed to load search results. Please try again.</p>';
            }
        });
    </script>
